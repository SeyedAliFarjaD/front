...
function PageCountry() {
  ...
  let cityChart = (root) => {
    ...
    polygonSeries.mapPolygons.template.events.on("click", (ev) => {
      ...
      toast(`City ${dataItem.dataContext.name} selected`)

    });


    loadGeodata = function (country) {
 ...
 */
    ...
    var root2 = am5.Root.new("chartCityDiv");
  ...
  useLayoutEffect(() => {
    ...
    let root2 = am5.Root.new("chartCityDiv");
    ...
    //   Promise.all([
    //     zoomAnimation.waitForStop(),
    //     am5.net.load("https://cdn.amcharts.com/lib/5/geodata/json/" + data.name.toLocaleLowerCase() + 'Low' + ".json", chart)
    //   ]).then((results) => {
    //     var geodata = am5.JSONParser.parse(results[1].response);
    //     // countrySeries.setAll({
    //     //   geoJSON: geodata,
    ...
    polygonSeries.mapPolygons.template.on("active", function (active, target) {
      ...
      if (target.get("active")) {
        ...
        if (centroid) {
          chart.animate({ key: "rotationX", to: -centroid.longitude, duration: 700, easing: am5.ease.inOut(am5.ease.cubic) });
          chart.animate({ key: "rotationY", to: -centroid.latitude, duration: 700, easing: am5.ease.inOut(am5.ease.cubic) });
        }


        // target.dataItem.dataContext.name
        // target.dataItem.dataContext.id
        toast(`Country ${target.dataItem.dataContext.name} selected`)
        loadGeodata(target.dataItem.dataContext.id)
  ...
  return (
    ...
    <Container component="main" className='TextField-with-border-radius' sx={{ padding: '16px 8px' }}>
      <Card sx={{
        position: 'relative',
        overflow: 'visible',
        maxWidth: '968px',
        ...
        <div style={{
          ...
          <div id="chartdiv" style={{ width: '100%', height: "458px", marginRight: '48px' }}></div>
          <div>
            <div>

              <div id="chartCityDiv" style={{ width: '100', height: "368px" }}></div>
              ...
              <Button
                ...
                sx={{ mt: 3, mb: 1, mr: 4, borderRadius: '108px', minHeight: '', minWidth: '128px', backgroundColor: '#3CA700' }}>
        ...
        {/* <Button
          ...
          variant="contained"
          onClick={handleOnNextClick}
          sx={{ pr: 10, pl: 10, mt: 3, mb: 1, borderRadius: '108px', minHeight: '', minWidth: '128px' }}>
          Change Language
